...
elseif ESX.GetConfig().Multichar == true then
	...
	local function SetupCharacters(source)
		...
		if result then
			...
			for i = 1, characterCount, 1 do
				...
				local job2, grade2 = v.job or 'none', tostring(v.job2_grade)
				...
				if ESX.Jobs[job2] and ESX.Jobs[job2].grades[grade2] then
					if job2 ~= 'none' then grade2 = ESX.Jobs[job2].grades[grade2].label else grade2 = '' end
	...
	MySQL.ready(function()
		...
		if DB_COLUMNS then
			...
			FETCH = 'SELECT identifier, accounts, job, job_grade, job2, job2_grade, firstname, lastname, dateofbirth, sex, skin, disabled FROM users WHERE identifier LIKE ? LIMIT ?'
		end
	end)
